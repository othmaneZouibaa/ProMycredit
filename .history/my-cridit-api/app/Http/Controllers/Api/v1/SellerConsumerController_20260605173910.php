...
class SellerConsumerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Fetch consumers with their credits stats (unpaid count, total remaining)
        $consumers = $query->with(['credits'])->get()->map(function ($consumer) {
            $credits = $consumer->credits;
            
            $totalCreditsCount = $credits->count();
            $unpaidCreditsCount = $credits->where('status', '!=', 'paid')->count();
