...
class ConsumerCreditController extends Controller
{
    ...
    public function accept($id)
    {
        ...
        // Find the credit that belongs to a consumer profile matching this user
        $cleanPhone = preg_replace('/[^0-9]/', '', $user->phone);
        $consumerIds = Consumer::where('user_id', $user->id)
            ->orWhere(function($query) use ($cleanPhone, $user) {
                if ($cleanPhone) {
        ...
        // Permanently link this consumer profile to this user if not already linked
        $consumer = Consumer::find($credit->consumer_id);
        if (!$consumer->user_id) {
