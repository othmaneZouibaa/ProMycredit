...
const ListConsumers = () => {
    ...
    function handleConfirmeRemove(list) {
        if (window.confirm('Est ce que vous voullez supprimer : ' + tuto.titre)) {
            dispatch(remove(tuto.code))
        }
    }
  return (
    <div>
            ...
            <tbody>
                {cl.map((item,index)=>(
                    ...
                    <td>
                      ...
                      <button className="btn btn-delete" onClick={()=>handleConfirmeRemove(item)} >Supprimer</button>
                    </td>
                  </tr>
                ))}
